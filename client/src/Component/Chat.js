...
function Chat() {
    ...
    return (
        <div id="chatContainer">채팅
            <div className="nowChat">
                {nowChat ? <div><img id="nowChat" src="https://media3.giphy.com/media/9rv3RJwRmXWkISOUQc/giphy.gif?cid=ecf05e479nfusr0h4m7inyn3k4xeszsk00zzcj0b7z55gxjx&rid=giphy.gif&ct=g" />{nowChatUser}</div> : null}
            </div>
            <ul id="messages">
                {chatMsg.length !== 0 ? chatMsg.map(item =>
                    <li className="chat">
            ...
            <form id="form" action="" onSubmit={testFun}>
                <input id="input" autocomplete="off" onKeyUp={keyTestFun} /><button>Send</button>
            </form>
        </div>
    );
}
