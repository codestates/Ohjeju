...
function KakaoMap() {
  ...
  manager.addListener('drawend',function(data){ //네모에 이벤트추가
...
const getCategory = (category) => {
    ...
    //위치범위세팅
    if(area.length !== 0){ //영역선택안하고 카테고리눌르면 에러나니까
        //평상시에는 안보여야겟지?
      ...
      const callback = function(result,status,pagination) {
        ...
        const markerIcon = createMarkerImage();
          if(status === window.kakao.maps.services.Status.OK){//마킹시작
            result.forEach((item,idx) => {//result 받은거를 각각
              ...
              geocoder.coord2Address(coord.getLng(),coord.getLat(),(result,status)=>{
                if(status === window.kakao.maps.services.Status.OK){
                  axios.post('http://localhost:80/attractions',{
                    location:result[0].address.address_name
                  })
                  .then(ele =>  {
                    let img = 'https://blog.kakaocdn.net/dn/o1KIw/btqu9mflPY6/rGk1mM3iugV1c6jj9Z3E80/img.jpg'
                    if(ele.data !== null) {img = ele.data.image}
                    const listItem = getListItem(idx,item,img)
                    listItem.onmouseover = () => displayInfowindow(infowindow,marker,item)
            ...
            // -----forEach 종료-----
            if(pagination.hasNextPage){ //다음페이지
              pagination.nextPage();
            }
          }
      }
      if(hasKeyWord.length !== 0){
        ...
        places.categorySearch(category,callback,{ //네모범위안 카테고리검색
          bounds:bounds, //네모사이즈
          size:15,
          page:1
        })
    ...
    else{alert('영역을 선택해주세요')}
  }

  const createMap = () => { //스토리지를 클리어하고 실행을해야 새로고침해도 작동이됨 순서가있는듯함
    const mapOption = {
      center: new window.kakao.maps.LatLng(33.379380, 126.543917),
      level: 10 //소수점단위안됨
    }
  ...
  function displayInfowindow(infowindow,marker, item) {
    let overLayContent = '<div class="placeinfo">' +'<a class="title" href="'+item.place_url+'" target="_blank" title="'+item.place_name+'">'+item.place_name +'</a>';
    if (item.road_address_name) {
                overLayContent += '<span class="roadAddress>' + item.road_address_name + '</span>' +
              '<span class="jibun">(지번 : ' + item.address_name + ')</span>';}
              else {overLayContent += '<span title="' + item.address_name + '">' + item.address_name + '</span>';}
              overLayContent += '<span class="tel">' + item.phone + '</span>' + '</div>' + '<div class="after"></div>';
    infowindow.setContent(overLayContent);
    infowindow.open(map, marker);
}
  function createOverLay(item,point){
    let overLayContent = '<div class="placeinfo" style="margin-bottom:200px">'
    +'<a class="title" href="'+item.place_url+'" target="_blank" title="'+item.place_name+'">'+item.place_name+'</a>'
    +'<div id="close"></div>';
    if (item.road_address_name) {
    overLayContent += '<span class="roadAddress>' + item.road_address_name + '</span>' +'<span class="jibun">(지번 : ' + item.address_name + ')</span>';}
    else {overLayContent += '<span title="' + item.address_name + '">' + item.address_name + '</span>';}
    overLayContent += '<span class="tel">' + item.phone + '</span>' + '</div>' + '<div class="after"></div>';

    return new window.kakao.maps.CustomOverlay({content:overLayContent,position:point});
  }
  ...
  function getListItem(index, places, setImage) {
    ...
    itemStr = '<img class="markerbg marker_'+(index+1)+
              '"src="'+(setImage)+'"></img>' +
                '<div class="info">' +
  ...
  return (
    <div>
      {/* 카테고리는 추가가능 */}
      <ul id="category">
        <li id="CE7" onClick={() => getCategory('CE7',1)}>
          카페
        </li>
        <li id="AT4" onClick={() => getCategory('AT4',1)}>
          관광명소
        </li>
        <li id="FD6" onClick={() => getCategory('FD6',1)}>
          음식점
        </li>
        <li onClick={drawTool}>
          사각형 그리기
        ...
        <div id="map" style={mapStyle}></div>
          <div id="menu_wrap" className="bg_white close">
            <div className="option">
              키워드 : <input id="keyword" size="15" type="text" onKeyUp={testKeydown}/><button onClick={getCategory}>검색</button>
            </div>
            <hr></hr>
            <ul id="placesList">
            </ul>
